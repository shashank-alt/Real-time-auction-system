...
export const supa = (SUPABASE_URL && SUPABASE_SERVICE_KEY)
    ...
    : null;
export async function createAuction(body, sellerId) {
    if (!supa)
...
export async function listAuctionsBySeller(sellerId) {
    if (!supa)
        return { status: 200, body: { items: [] } };
...
export async function placeBid(auctionId, userId, amount) {
    ...
    if (e1 || !a)
        return { status: 404, body: 'Auction not found' };
    const nowIso = new Date().toISOString();
    if (a.status === 'closed' || nowIso > a.endsAt)
        return { status: 409, body: 'Auction ended' };
    ...
    const { data: updated, error: e2 } = await supa
        ...
        .select();
    if (e2)
        return { status: 500, body: e2.message };
    if (!updated || updated.length === 0)
        return { status: 409, body: 'Bid too low or auction ended' };
...
export async function endAuction(auctionId, sellerId) {
    if (!supa)
        return { status: 500, body: 'Supabase not configured' };
    ...
    if (a.sellerId !== sellerId)
        return { status: 403, body: 'Not seller' };
...
export async function decision(auctionId, sellerId, action, amount) {
    if (!supa)
        return { status: 500, body: 'Supabase not configured' };
    ...
    if (!tb)
        return { status: 409, body: 'No bids' };
    if (action === 'accept') {
    ...
    if (!amount)
        return { status: 400, body: 'Counter amount required' };
    ...
    if (ce)
        return { status: 500, body: ce.message };
...
export async function counterReply(counterId, userId, accept) {
    if (!supa)
        return { status: 500, body: 'Supabase not configured' };
    ...
    if (!c)
        return { status: 404, body: 'Not found' };
    if (c.buyerId !== userId)
        return { status: 403, body: 'Not buyer' };
...
export async function startAuction(auctionId, minutes) {
    if (!supa)
        return { status: 500, body: 'Supabase not configured' };
    const { data: a } = await supa.from('auctions').select('*').eq('id', auctionId).single();
    if (!a)
...
export async function resetAuction(auctionId, minutes) {
    if (!supa)
        return { status: 500, body: 'Supabase not configured' };
    const { data: a } = await supa.from('auctions').select('*').eq('id', auctionId).single();
    if (!a)
...
export async function getTopBid(auctionId) {
    if (!supa)
        return null;
